...
def best_value
  ...
  execute(<<-SQL)
  SELECT
    title, price, COUNT(tracks.song) AS num_tracks 
  FROM
    albums
  JOIN
    tracks ON tracks.album = albums.asin
...
def rock_superstars
  ...
  execute(<<-SQL)
  SELECT
    artist, COUNT(albums.asin) AS num_albums
  FROM
    albums
  JOIN
